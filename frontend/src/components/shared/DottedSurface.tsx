...
export function DottedSurface({ className, style, ...props }: DottedSurfaceProps) {
	...
	useEffect(() => {
		...
		for (let ix = 0; ix < AMOUNTX; ix++) {
			for (let iy = 0; iy < AMOUNTY; iy++) {
				...
				// Brand slate #9EACCA → rgb(158, 172, 202) normalized
				colors.push(158 / 255, 172 / 255, 202 / 255);
			}
		}
	...
	return (
		<div
			...
			style={{ position: 'absolute', inset: 0, zIndex: 0, pointerEvents: 'none', overflow: 'hidden', ...style }}
			{...props}
		/>
	);
}
